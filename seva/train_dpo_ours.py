...
from typing import Dict, Optional, List, Sequence, Literal
...
@dataclass
class DataArguments:
    ocr_data_path: str = field(default=None, metadata={"help": "Path to the training ocr json files."})
    ocr_image_path: str = field(default=None, metadata={"help": "Path to the training ocr image folders."})
    textvqa_data_path: str = field(default=None, metadata={"help": "Path to the training textvqa json files."})
    textvqa_image_path: str = field(default=None, metadata={"help": "Path to the training textvqa image folders."})
...
def setup_llava_model(model_args, data_args, script_args):
    ...
    ### Initialize LLM (some version also include MLP)
    ### 1. LLM's params, all requires_grad=True
    ### 2. MLP's params, all requires_grad=True
    ...
    if model_args.vision_tower is not None:
        ...
        ### Initialize Vision Tower, with LORA-LLM 
        ### 1. LLM's exclude {token_embed, lm_head} requires_grad=True
        ### 2. MLP's params requires_grad=True
        ### 3. Vision's params requires_grad=False
        # for k,v in model.named_parameters():
        #     if v.requires_grad == True:
        #         print("True", k)
        #     else:
