...
# CUDA_VISIBLE_DEVICES=4 OMP_NUM_THREADS=8 python llava/eval/calculate_match_score.py \
#     --quilt True \
#     --gt  ./playground/dpo_data/step2/matched_20k_gt.jsonl \
#     --pred  ./playground/dpo_data/vllm_generate/matched_20k_Qwen2-VL-7B-Instruct_t0.1.jsonl
...
def evaluate(gt, pred, quilt=False, anchor=None):  
    ...
    for gt_item, pred_item, anchor_item in zip(gt, pred, anchor if anchor else pred):
        ...
        # rougeL_scores['hit'].append(rouge.compute(predictions=pred_value, references=gt_value)['rougeL'])
...
if __name__ == '__main__':
    ...
    gt_ids = [item['image_id'] for item in gt]
    pred_ids = [item['image_id'] for item in pred]
