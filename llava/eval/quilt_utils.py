...
def token_edit_distance(seq1, seq2):
    ...
    # 创建一个 (len1+1) x (len2+1) 的矩阵，用于存储动态规划的结果
    dp = [[0] * (len2 + 1) for _ in range(len1 + 1)]

    # 初始化第一行和第一列
    for i in range(1, len1 + 1):
        dp[i][0] = i  # seq1 的前 i 个字符到空串的距离是 i（删除操作）

    for j in range(1, len2 + 1):
        dp[0][j] = j  # 空串到 seq2 的前 j 个字符的距离是 j（添加操作）
...
def calculate_f1score(candidate, reference):
    ...
    for word in word_set:
        if word in candidate_words and word in reference_words:
            ...
            tn += reference_words[word]
    
    if len(candidate_words) == 0:
        return 0, 0, 0, 0 # "0 (warning: length of candidate's words is 0)"
        ...
        if tp == 0:
            ...
            return 2 * precision * recall / (precision + recall), precision, recall, acc